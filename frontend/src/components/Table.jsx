...
function Table(props) {
  ...
  return (
    <>
      <div className='sm:flex flex-row sm:gap-x-2'>
        <GlobalFilter
        ...
        <div
          className={classNames(
            ...
            selected != -1 && 'bg-red-200 cursor-pointer hover:bg-red-300'
          )}
        >
          <TrashIcon className='h-8 w-8 m-1 text-slate-500' />
        </div>
        <div
          className={classNames(
            'bg-white rounded shadow border border-slate-200',
            selected != -1 && 'bg-lime-200 cursor-pointer hover:bg-lime-300'
          )}
        >
          <PencilIcon className='h-8 w-8 m-1 text-slate-500' />
        </div>
      </div>
      {/* table */}
                  ...
                  {page.map((row, i) => {
                    ...
                    return (
                      <tr
                        {...row.getRowProps()}
                        className={classNames(
                          selected == row.original.id && 'bg-slate-100'
                        )}
                        onClick={() => {
                          if (selected == row.original.id) {
                            setSelected(-1)
                          } else {
